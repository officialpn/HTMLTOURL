...
document.querySelectorAll('.expiry-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        selectedExpiry = btn.dataset.days;
    });
});
        ...
        if (xhr.status === 200) {
            ...
            setTimeout(() => {
                ...
                document.getElementById('resultFileName').textContent = 
                    data.filename.length > 20 ? data.filename.substring(0, 17) + '...' : data.filename;
                document.getElementById('resultFileSize').textContent = data.size;
                document.getElementById('resultExpiry').textContent = data.expiry + ' days';
            }, 600);
        } else {
